...
{
    public class RaceUndeadScourge : CharacterRace
    {
        ...
        public override void Register()
        {
            AddAbility(
                new SimpleWarcraftAbility(
                    ...
                    "Vampiric Aura",
                    i =>
                        $"Heal for {ChatColors.Green}10/20/30/40/50%{ChatColors.Default} of damage dealt to enemies."
                )
            );
            AddAbility(
                new SimpleWarcraftAbility(
            ...
            AddAbility(
                new SimpleWarcraftAbility(
                    ...
                    "Levitation",
                    i =>
                        $"Experience {ChatColors.LightBlue}90/80/70/60/50%{ChatColors.Default} of gravity."
                )
            );
            AddAbility(
                new SimpleWarcraftAbility(
        ...
        private void PlayerHurtOther(GameEvent @obj)
        {
            ...
            int vampiricAuraLevel = PlayerCharacter.GetAbilityLevel(0);
            ...
            if (victim != null & !string.IsNullOrEmpty(victim.PlayerName) && amountToHeal > 0)
                PlayerCharacter.SetStatusMessage($" {ChatColors.Green}+{amountToHeal} HP");
        ...
        private void PlayerSpawn(GameEvent @event)
        {
            ...
            int levitationLevel = PlayerCharacter.GetAbilityLevel(2);
            ...
            int unholyAuraLevel = PlayerCharacter.GetAbilityLevel(1);
        ...
        private void PlayerDeath(GameEvent @obj)
        {
            ...
            if (
                PlayerCharacter.GetAbilityLevel(3) > 0
                && @event.Userid.Handle != @event.Attacker.Handle
            )
            {
